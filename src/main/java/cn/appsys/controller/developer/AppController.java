...
public class AppController {
	...
	public String devAccountApply(){
		return "developer/devapply";
	}
	@RequestMapping(value="/register",method=RequestMethod.POST)
	public String registAccount(HttpServletRequest request,@RequestParam(value="appDevCode") String appDevCode,
								@RequestParam(value="appDevName") String appDevName,
								@RequestParam(value="inputPassword3") String inputPassword3,
								@RequestParam(value="devEmail") String devEmail){
		
		logger.info("获取信息 -- > appDevCode: " + appDevCode);
		logger.info("获取信息 -- > appDevName: " + appDevName);
		logger.info("获取信息 -- > inputPassword3: " + inputPassword3);
		logger.info("获取信息 -- > devEmail: " + devEmail);
		
		DevUser devUser=new DevUser();
		devUser.setDevCode(appDevCode);
		devUser.setDevName(appDevName);
		devUser.setDevEmail(devEmail);
		devUser.setDevPassword(inputPassword3);
		
		
		
		boolean flag=false;
		try {
			flag=appInfoService.appRegister(devUser);
		} catch (Exception e) {
		...
		if(flag){
			...
			return "developer/devapply";//成功
		}
		request.setAttribute("error", "保存失败");
		return "developer/devapply";//失败
	}
